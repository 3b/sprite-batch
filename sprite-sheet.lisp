...
(defclass tileset ()
  ((texture-names :initform nil :accessor textures)
   ...
   ;; general info based on tiled/tmx tileset tag
...
(defclass atlas-tileset (tileset)
...
(defmethod %load-tileset ((tileset atlas-tileset))
...
(defun load-atlas (image tile-width tile-height
                   &key (spacing 0) (margin 0)
                     columns (tile-offset #(0.0 0.0))
                     (first-id 1) name source
                     ;; fixme: better API for loading from file vs buffer
                     data data-format)
  (let* ((tileset (make-instance 'atlas-tileset
                                 :first-id first-id
                                 :source source
                                 :name name
    ...
    (if image
        ...
        (flex:with-input-from-sequence
          ...
          (load-texture-stream tileset s (make-keyword data-format))))
    (let ((te (list tileset :tile-extents)) ;; names of buffer textures
      ...
      (setf (textures tileset) (list (or image tileset) te to))
      (%load-tileset tileset)
      tileset)))

(defun load-atlas/tmx (tmx-tileset)
  (destructuring-bind (&key attributes image tile-offset
    ...
    (flet ((p (n &optional (p attributes))
      ...
      (let ((image-props (getf (first image) :attributes)))
        (load-atlas (p :source image-props) (p :tile-width) (p :tile-height)
                    ...
                    :tile-offset (or tile-offset #(0.0 0.0))
                    :data (p :data image-props)
                    :data-format (p :format image-props))))))

(defmethod bind ((ts atlas-tileset))
